...
class ExtractHumanPoseLandmarks(
    ...
    def execute(self, input: LandmarkingStageInput) -> List[PoseLandmarkerResult]:
        ...
        for frame in input.video_data.frames:
            mp_image = mp.Image(image_format=mp.ImageFormat.SGRB, data=frame.frame)
            pose_landmarker_result = landmarker.detect_for_video(
                mp_image, frame.timestamp_ms
            )
            landmaker_results.append(pose_landmarker_result)

        return landmaker_results