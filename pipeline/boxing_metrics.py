...
class CalculateBoxingMetrics(
    ...
    def calculate_com(self, position: np.ndarray):
        ...
        pos={
            ...
            'upper_arm_R': lambda: get_midpoint(PoseLandmark.RIGHT_SHOULDER,PoseLandmark.RIGHT_ELBOW, 0.436, 0.564),
            'upper_arm_L': lambda: get_midpoint(PoseLandmark.LEFT_SHOULDER,PoseLandmark.LEFT_ELBOW, 0.436, 0.564),
            'lower_arm_R': lambda: get_midpoint(PoseLandmark.RIGHT_ELBOW,PoseLandmark.RIGHT_WRIST, 0.43, 0.57),
            'lower_arm_L': lambda: get_midpoint(PoseLandmark.LEFT_ELBOW,PoseLandmark.LEFT_WRIST, 0.43, 0.57),
            'hand_R': lambda : position[:, PoseLandmark.RIGHT_WRIST],
            'hand_L': lambda : position[:, PoseLandmark.LEFT_WRIST],
            'thigh_R': lambda: get_midpoint(PoseLandmark.RIGHT_HIP,PoseLandmark.RIGHT_KNEE, 0.43, 0.57),
            'thigh_L': lambda: get_midpoint(PoseLandmark.LEFT_HIP,PoseLandmark.LEFT_KNEE, 0.43, 0.57),
            'calf_R': lambda: get_midpoint(PoseLandmark.RIGHT_KNEE,PoseLandmark.RIGHT_ANKLE, 0.43, 0.57),
            'calf_L': lambda: get_midpoint(PoseLandmark.LEFT_KNEE,PoseLandmark.LEFT_ANKLE, 0.43, 0.57),
            'foot_R': lambda: position[:, PoseLandmark.RIGHT_ANKLE],
            'foot_L': lambda: position[:, PoseLandmark.LEFT_ANKLE]

        }
        get_midpoint = lambda l,r, lw=1.,rw=1.: (position[:, l] * lw+ position[:, r] * rw) / (lw+rw)    
        def get_torso():
            top_torso = get_midpoint(PoseLandmark.RIGHT_SHOULDER,PoseLandmark.LEFT_SHOULDER)
            bottom_torso = get_midpoint(PoseLandmark.RIGHT_HIP,PoseLandmark.LEFT_HIP)
            return (top_torso + bottom_torso) / 2
        com_f = lambda key: mass[key] * pos[key]()
        com_rl = lambda key, side: mass[key] * pos[f"{key}_{side}"]()
        com = com_f('head') + com_f('torso') + \
            com_rl('upper_arm', 'R') + com_rl('upper_arm', 'L') + \
            com_rl('lower_arm', 'R') +com_rl('lower_arm', 'L') + \
            com_rl('hand', 'R')+ com_rl('hand', 'L') + \
            com_rl('thigh', 'R') + com_rl('thigh', 'L') + \
            com_rl('calf', 'R') + com_rl('calf', 'L') + \
            com_rl('foot', 'R') + com_rl('foot', 'L')
